...
if [ "${CLUSTER_NUM_NODES}" -gt 1 ]; then
    ...
    TRAIN_NCCL_ENV="{\"NCCL_SOCKET_IFNAME\":\"${NCCL_IF}\",\"GLOO_SOCKET_IFNAME\":\"${NCCL_IF}\",\"NCCL_IB_DISABLE\":\"1\",\"NCCL_P2P_DISABLE\":\"${NCCL_P2P_DISABLE:-1}\",\"NCCL_NVLS_ENABLE\":\"${NCCL_NVLS_ENABLE}\",\"NCCL_DEBUG\":\"${NCCL_DEBUG:-WARN}\",\"TORCH_SHOW_CPP_STACKTRACES\":\"1\"}"
fi
