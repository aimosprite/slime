#!/bin/bash -l
#SBATCH --job-name=slime-scaff-full-20b-32k
#SBATCH --output=/home/rohin/slime/examples/scaffolding/slime-scaffolding-full-20b-32k-%j.log
#SBATCH --error=/home/rohin/slime/examples/scaffolding/slime-scaffolding-full-20b-32k-%j.log
...
# Small *full* scaffolding RL run: real JSONL (no smoke truncation), default 8 attempts + judge,
# non-debug training (no --debug-rollout-only), Weights & Biases logging.
# Rollout context cap: 32768 (32 Ki, power of two). Use the *_16k_ctx* script if this OOMs.
#
# Credentials: optional file examples/scaffolding/.env (WANDB_API_KEY, HF_TOKEN).
# Paths: override SLIME_SCRIPT_HF_CHECKPOINT / SLIME_SCRIPT_DATA_JSONL if needed.
#
#   sbatch examples/scaffolding/sbatch_train_small_full_mit_orcd.sh

set -euo pipefail

# Slurm copies this script under /var/spool/slurmd/ — do not use BASH_SOURCE for .env location.
...
# Do not skip wandb (in case it was exported in the shell).
unset SLIME_SCRIPT_SKIP_WANDB || true

# --- GPT-OSS 20B, 2x H200, Megatron bridge (matches launcher defaults for TP/EP) ---
...
export SLIME_SCRIPT_GLOBAL_BATCH_SIZE="${SLIME_SCRIPT_GLOBAL_BATCH_SIZE:-16}"

export SLIME_SCRIPT_ROLLOUT_MAX_RESPONSE_LEN="${SLIME_SCRIPT_ROLLOUT_MAX_RESPONSE_LEN:-8192}"
export SLIME_SCRIPT_ROLLOUT_MAX_CONTEXT_LEN="${SLIME_SCRIPT_ROLLOUT_MAX_CONTEXT_LEN:-32768}"
